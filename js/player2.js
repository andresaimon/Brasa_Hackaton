const MAX_PLAYER_HP2 = 4;
const INITIAL_LIFES2 = 3;
...
class Player2 {
  constructor(image) {
    ...
    this.hp = MAX_PLAYER_HP2;
    this.damageValue = 1;
    this.lifes = INITIAL_LIFES2;
  ...
  draw() {
    if (!this.dead) {
      ...
      if (this.invencible) {
        let invencibleAura2 = colors.GOLD;
        invencibleAura2.setAlpha(50 + (32 * sin(millis() / 1000)));
        fill(invencibleAura2);
        noStroke();
        circle(this.position.x + this.size.width/2, this.position.y + this.size.height/2, this.size.width/0.8);
        invencibleAura2.setAlpha(255);
      } else {
  ...
  handleShots() {
    if (keyIsDown(ZERO_KEYCODE) && !this.shooting) {
      this.shooting = true;
      ...
      if (this.upgrades.active.multiCannons) {
        ...
        player2Bullets.push(ShotUp);
        player2Bullets.push(ShotDown);
      }
