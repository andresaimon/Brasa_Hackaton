class HelpMenu {
  ...
  draw() {
    ...
    text("Game Play", this.currentTextPosition.x + offsetX, this.currentTextPosition.y + offsetY);
    stroke(colors.BLACK);
    fill(colors.RED);
    text("Game Play", this.currentTextPosition.x, this.currentTextPosition.y);
    
    fill(color(255, 255, 255));
    textFont(ASSETS.fonts.orbitron);
    textSize(26);
    text("Player1 - Use WASD para controlar e espaco para atirar", width / 2, height - (height / 2));
    text("Player2 - Use as setas para controlar e zero para atirar", width / 2, height - (height / 2.3));
