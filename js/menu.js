class Menu {
  ...
  draw() {
    ...
    text("independence", this.currentTextPosition.x + offsetX, this.currentTextPosition.y + offsetY)
    fill(colors.GOLD);
    stroke(colors.BLACK);
    text("independence", this.currentTextPosition.x, this.currentTextPosition.y)
    
    
    let offsetX2 = ((width - mouseX) / width * 5);
    let offsetY2 = ((height - mouseY) / height * 5 + 100);
    text("space", this.currentTextPosition.x + offsetX2, this.currentTextPosition.y + offsetY2)
    fill(colors.GOLD);
    stroke(colors.BLACK);
    text("space", this.currentTextPosition.x + offsetX2, this.currentTextPosition.y + offsetY2)
  ...
  startGame() {
    ...
    if (!ASSETS.sounds.inGame.isLooping()) {
      ASSETS.sounds.inGame.setVolume(0.3);
